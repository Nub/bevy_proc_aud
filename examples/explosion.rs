...
fn ui_system(
    ...
    egui::Window::new("Explosion / Fireball").show(ctx, |ui| {
        ...
        ui.separator();
        if ui.button("Deep Explosion (0.5x)").clicked() {
        ...
        if ui.button("Standard Explosion").clicked() {
            commands.spawn(Explosion {
                intensity: settings.intensity,
        ...
        if ui.button("Fireball (2x)").clicked() {
            commands.spawn(Explosion {
                intensity: settings.intensity,
                pitch_shift: 2.0,
                reverb_mix: settings.reverb,
        ...
        if ui.button("Small Fireball (3x)").clicked() {
            commands.spawn(Explosion {
                intensity: settings.intensity,
                pitch_shift: 3.0,
                reverb_mix: settings.reverb,
